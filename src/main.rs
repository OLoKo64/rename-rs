...
fn execute(args: &AppArguments, entries: &[std::path::PathBuf]) -> Result<(), std::io::Error> {
    if args.no_act {
...
fn only_dirs_entries(paths: Vec<std::path::PathBuf>) -> Vec<std::path::PathBuf> {
    paths.into_iter().filter(|p| p.is_dir()).collect()
}

fn only_files_entries(paths: Vec<std::path::PathBuf>) -> Vec<std::path::PathBuf> {
    paths.into_iter().filter(|p| p.is_file()).collect()
}

fn all_files_in_dir(pattern: &str) -> Result<Vec<std::path::PathBuf>, GlobErrors> {
